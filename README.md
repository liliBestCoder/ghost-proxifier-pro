...
| :--- | :---: | :---: | :---: | :---: |
| **底层实现原理** | Winsock API Hook (DLL 注入) | WFP 内核驱动 / Winsock | WinDivert 包拦截驱动 | DLL 动态注入 / API Hook |
| **内核驱动要求** | ❌ 无需驱动 (纯用户态注入) | ⚠️ 需安装 WFP 驱动 | ⚠️ 需安装 WinDivert 驱动 | ❌ 无需驱动 (纯用户态) |
| **适用软件范围** | ✅ 通用多应用管理 (20+ 常用软件) | ✅ 通用多应用匹配 | ✅ 通用应用分流 | ⚠️ 专为 Antigravity 针对性开发 |
| **界面与易用性** | ✅ 现代化 GUI (拖拽自动注入) | 经典传统桌面 GUI | 客户端 GUI / CLI | 基础 CLI / 简单界面 |
| **进程树自动追踪** | ✅ 自动递归覆盖所有子进程 | ✅ 基于规则名模式匹配 | ⚠️ 基于进程名过滤 | ❌ 仅针对单一目标进程 |
| **DoH / QUIC 自动阻断** | ✅ 内置主动阻断 (强制 TCP 走代理) | ❌ 需配合防火墙规则 | ❌ 无内置阻断机制 | ❌ 无内置阻断机制 |
