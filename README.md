...
是的，非pro用户不可使用拖拽快捷方式或者exe程序直接注入，只可使用手动选择框选择要注入的程序的快捷方式或者exe，最多可注入3个应用程序，且单次注入应用运行时长不可超过2h，超过2h就需要重启才可继续使用。pro付费用户则没有这些限制，想拖入多少应用就拖入多少，且不限制单次运行时间。pro用户收费标准，请参见官网。
