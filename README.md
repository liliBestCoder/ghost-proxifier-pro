...
| :--- | :---: | :---: | :---: | :---: |
| **技术实现原理** | Winsock API Hook (DLL注入) | WFP 内核驱动 / Winsock | 本地代理网桥 / 端口转发 | DLL 动态注入 / API Hook |
| **内核驱动要求** | 无需驱动 (纯用户态) | 需安装 WFP 系统驱动 | 无需驱动 | 无需驱动 (纯用户态) |
| **界面交互** | Pro 现代图形界面 (拖拽即用) | 经典传统 GUI | 命令行 (CLI) / 配置文件 | 基础 GUI / 命令行 |
| **进程树自动分流** | ✅ 自动追踪并注入子进程 | ✅ 基于规则与进程名匹配 | ❌ 依赖系统代理/环境变量 | ⚠️ 须指定目标进程注入 |
