...
| :--- | :---: | :---: | :---: | :---: |
| **技术实现原理** | Winsock API Hook (用户态) | WFP 内核驱动 / Winsock | DLL 动态注入 | TUN/TAP 虚拟网卡 / 本地代理 |
| **内核驱动要求** | 无需驱动 (纯用户态注入) | 需安装 WFP 系统驱动 | 无需驱动 | 需安装 Wintun 虚拟网卡驱动 |
| **界面交互** | Pro 现代图形界面 (拖拽即用) | 经典传统 GUI | 命令行 (CLI) / 配置文件 | 基础 GUI / Web 界面 |
| **进程树自动分流** | ✅ 自动追踪并注入子进程 | ✅ 基于规则与进程名匹配 | ⚠️ 须手动或单进程注入 | ❌ 依赖全局/虚拟网卡分流 |
| **DoH / QUIC 自动阻断** | ✅ 内置主动阻断 (强退 TCP) | ❌ 须手动配置防火墙规则 | ❌ 无阻断机制 | ❌ 无阻断机制 |
| **软件形式** | 桌面独立客户端 | 商业收费软件 | 轻量级工具 | 开源/社区代理工具 |
